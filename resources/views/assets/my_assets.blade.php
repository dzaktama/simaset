            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        ...
                        {{-- Row tetap bisa diklik (UX Modern), tapi tombol juga ada (UX Explicit) --}}
                        <tr class="hover:bg-gray-50 transition cursor-pointer group" onclick="openDetailModal({{ json_encode($asset) }})">
                            {{-- Info Aset --}}
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <div class="h-12 w-12 flex-shrink-0 rounded-lg bg-gray-100 border border-gray-200 overflow-hidden relative">
                                        @if($asset->image)
                                            <img class="h-full w-full object-cover" src="{{ asset('storage/' . $asset->image) }}" alt="Foto Aset">
                                        @else
                                    ...
                                    <div class="ml-4">
                                        <div class="text-sm font-bold text-gray-900 group-hover:text-indigo-600 transition">{{ $asset->name }}</div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <div class="text-xs text-gray-500">{{ $asset->location ?? 'Umum' }}</div>
                            </td>

                            {{-- Tanggal Pinjam --}}
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900">
                                    {{ \Carbon\Carbon::parse($asset->assigned_date)->translatedFormat('d M Y') }}
                                </div>
                                <div class="text-xs text-gray-500">
                                    {{ \Carbon\Carbon::parse($asset->assigned_date)->diffForHumans() }}
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium" onclick="event.stopPropagation()">
                                <div class="flex justify-end gap-2">
                                    {{-- 1. Tombol Detail (DIPULIHKAN) --}}
                                    <button onclick="openDetailModal({{ json_encode($asset) }})" 
                                    ...
                                    @else
                                        {{-- Cari ID Request Asal untuk proses return --}}
                                        @php
                                        ...
                                        @else
                                            {{-- Fallback jika data request hilang/terhapus --}}
