    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <?php if($assets->isEmpty()): ?>
            <div class="p-12 text-center flex flex-col items-center">
                ...
                <a href="/assets" class="mt-5 inline-flex items-center px-5 py-2.5 border border-transparent text-sm font-medium rounded-lg text-white bg-indigo-600 hover:bg-indigo-700 shadow-sm transition">
                    Pinjam Aset Baru
                </a>
            </div>
        <?php else: ?>
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        ...
                        <tr class="hover:bg-gray-50 transition cursor-pointer group" onclick="openDetailModal(<?php echo e(json_encode($asset)); ?>)">
                            
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <div class="h-12 w-12 flex-shrink-0 rounded-lg bg-gray-100 border border-gray-200 overflow-hidden relative">
                                        <?php if($asset->image): ?>
                                            <img class="h-full w-full object-cover" src="<?php echo e(asset('storage/' . $asset->image)); ?>" alt="Foto Aset">
                                        <?php else: ?>
                                    ...
                                    <div class="ml-4">
                                        <div class="text-sm font-bold text-gray-900 group-hover:text-indigo-600 transition"><?php echo e($asset->name); ?></div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900"><?php echo e($asset->category ?? '-'); ?></div>
                                <div class="text-xs text-gray-500"><?php echo e($asset->location ?? 'Umum'); ?></div>
                            </td>

                            
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900">
                                    <?php echo e(\Carbon\Carbon::parse($asset->assigned_date)->translatedFormat('d M Y')); ?>

                                </div>
                                <div class="text-xs text-gray-500">
                                    <?php echo e(\Carbon\Carbon::parse($asset->assigned_date)->diffForHumans()); ?>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium" onclick="event.stopPropagation()">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <button onclick="openDetailModal(<?php echo e(json_encode($asset)); ?>)" 
                                    ...
                                    <?php
                                        // Cek apakah ada proses pengembalian yang pending
                                        $pendingReturn = \App\Models\AssetReturn::where('asset_id', $asset->id)
                                                        ->where('status', 'pending')->first();
                                    ?>

                                    <?php if($pendingReturn): ?>
                                        <span class="inline-flex items-center px-2.5 py-1.5 rounded-lg text-xs font-medium bg-yellow-50 text-yellow-700 border border-yellow-200 cursor-default">
                                            <svg class="w-3 h-3 mr-1.5 animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                            Menunggu Verifikasi
                                        </span>
                                    <?php else: ?>
                                        
                                        <?php
                                            $lastReq = \App\Models\AssetRequest::where('asset_id', $asset->id)
                                                        ->where('user_id', auth()->id())
                                                        ->where('status', 'approved')
                                                        ->latest()->first();
                                            $imgUrl = $asset->image ? asset('storage/' . $asset->image) : null;
                                        ?>
                                        
                                        <?php if($lastReq): ?>
                                            <button onclick="openReturnModal(<?php echo e($lastReq->id); ?>, '<?php echo e($asset->name); ?>', '<?php echo e($asset->serial_number); ?>', '<?php echo e($imgUrl); ?>', '<?php echo e($asset->assigned_date); ?>')" ...>
                                                Kembalikan
                                            </button>
                                        <?php else: ?>
                                            
                                            <span class="text-gray-300 text-xs italic cursor-not-allowed px-2">No Req Data</span>
                                        <?php endif; ?>
                                    <?php endif; ?>
                                </div>
                            </td>
        ...
        <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-md">
            <form action="<?php echo e(route('returns.store')); ?>" method="POST">
                ...
                <div class="px-6 py-4">
                    ...
                    <input type="hidden" name="asset_request_id" id="returnReqId">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="condition" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm" required>
                                ...
                                <option value="broken">Rusak Berat / Tidak Berfungsi</option>
                            </select>
                        </div>
...
<script>
    ...
    function openDetailModal(asset) {
        ...
        if (asset.assigned_date) {
            const d = new Date(asset.assigned_date);
